"""
Afet ve Acil Durum Yönetimi Başkanlığı KAYSİS Scraper Module
Afet ve Acil Durum Yönetimi Başkanlığı'nın KAYSİS sitesinden mevzuat tarama modülü
"""
import requests
...
def scrape_afet_acil_durum_mevzuat(url: str = "https://kms.kaysis.gov.tr/Home/Kurum/56388857") -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
    KAYSİS sitesinden Afet ve Acil Durum Yönetimi Başkanlığı mevzuatlarını tarar ve API ile karşılaştırır
    
    Args:
        url: Taranacak kurum URL'i (varsayılan: Afet ve Acil Durum Yönetimi Başkanlığı KAYSİS URL'i)
    ...
    """
    print(f"🔍 Afet ve Acil Durum Yönetimi Başkanlığı Mevzuat Tarama Başlatılıyor...")
    print(f"📡 Site: {url}")
...
def print_results_to_console(all_sections: List[Dict[str, Any]], stats: Dict[str, Any], uploaded_documents: Optional[List[Dict[str, Any]]] = None):
    ...
    """
    print("\n" + "="*80)
    print("📋 BULUNAN MEVZUATLAR (Afet ve Acil Durum Yönetimi Başkanlığı)")
    print("="*80)
