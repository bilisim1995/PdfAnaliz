"""
Boru Hatları İle Petrol Taşıma A.Ş. Genel Müdürlüğü KAYSİS Scraper Module
Boru Hatları İle Petrol Taşıma A.Ş. Genel Müdürlüğü'nün KAYSİS sitesinden mevzuat tarama modülü
"""
import requests
...
def scrape_boru_hatlari_petrol_tasima_mevzuat(url: str = "https://kms.kaysis.gov.tr/Home/Kurum/47423775") -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
    KAYSİS sitesinden Boru Hatları İle Petrol Taşıma A.Ş. Genel Müdürlüğü mevzuatlarını tarar ve API ile karşılaştırır
    
    Args:
        url: Taranacak kurum URL'i (varsayılan: Boru Hatları İle Petrol Taşıma A.Ş. Genel Müdürlüğü KAYSİS URL'i)
    ...
    """
    print("🔍 Boru Hatları İle Petrol Taşıma A.Ş. Genel Müdürlüğü Mevzuat Tarama Başlatılıyor...")
    print(f"📡 Site: {url}")
...
def print_results_to_console(all_sections: List[Dict[str, Any]], stats: Dict[str, Any], uploaded_documents: Optional[List[Dict[str, Any]]] = None):
    ...
    """
    print("\n" + "="*80)
    print("📋 BULUNAN MEVZUATLAR (Boru Hatları İle Petrol Taşıma A.Ş. Genel Müdürlüğü)")
    print("="*80)
